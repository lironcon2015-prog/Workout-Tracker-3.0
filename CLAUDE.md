...
- **מוקאפים — תמיד כ-Artifact מפורסם (קישור claude.ai), לא כקובץ.**
  - כתוב קובץ HTML ב-scratchpad ופרסם אותו בכלי `Artifact`; מסור בצ'אט את הקישור. אין להפנות לקובץ בריפו.
  - מבנה: מסגרות טלפון (~300px) זו לצד זו, כל אחת מצב/מסך אחד, עם כותרת קצרה מעליה והסבר של 1–2 שורות מתחתיה.
  - נאמנות לאפליקציה: טוקני הצבע, פונט Heebo, RTL ותוכן אמיתי (שמות תרגילים, משקלים) — לא lorem.
  - כל תוספת/שינוי מסומנים במסגרת מקווקוות ירוקה, כדי שיהיה ברור מה חדש מול מה שקיים.
  - כשההצעה כוללת גם שינוי התנהגות — מקטע נפרד באותו עמוד שמסביר מה משתנה.
  - עדכון מוקאפ באותה שיחה = פרסום מחדש לאותו קובץ (אותו קישור).
