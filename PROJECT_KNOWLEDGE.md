...
### 2. ששת בלוקי הגשרים — ~180 שורות העתק-הדבק

‏`index.html` מכיל שישה בלוקים כמעט זהים (status + מתג + URL + token + כפתורי פעולה),
נבדלים רק בקידומת ה-id, בתווית ובכפתור הנוסף. תבנות לרכיב אחד = refactor אמיתי עם סיכון
רגרסיה על מסך שכבר נשבר פעם. **לא לגעת בלי הוראה מפורשת ובדיקה ידנית של כל שישה.**
