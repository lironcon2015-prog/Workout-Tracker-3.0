...
- **תסמין 2 — "היה ייצוא אבל ה-PWA לא התעדכן":** שני כשלים **שקטים** ב-`_parseHAE`, שניהם תוקנו:
  1. **`date` חסר בלי Aggregate.** בפורמט הלא-מצטבר HAE שולחת `startDate`/`endDate` בלבד;
     `_isoDate(r.date)` החזיר null ו**כל השורות נזרקו בשקט** → `NO_DATA` גנרי. כעת הגשר
     **מסרב מפורשות** עם `HAE_NOT_AGGREGATED` + hint. **לא מסכמים דגימות גלם בכוונה** —
     זה בדיוק הבאג (מקורות חופפים) שבגללו עברנו ל-HAE.
  2. **ייחוס תאריך שגוי.** `date` של HAE = היום שבו הלילה **התחיל**; האפליקציה מייחסת שינה
     ל**תאריך הבוקר**. לילה שהתחיל ב-23:30 נשמר על אתמול → המסך נראה "לא מעודכן". כעת
     שורות שינה מיוחסות לפי **`sleepEnd`** (רגע היקיצה), עם נפילה ל-`date`/`endDate`/`startDate`.
