...
// _buildUnifiedSections — בונה משותף לששת המקטעים של הקובץ המאוחד, לטווח { from, to }
// (YYYY-MM-DD, null = בלי גבול). נצרך ע"י exportUnifiedData וע"י סנכרון נתוני המאמן
// לדרייב (coach-drive-logic.js) — מקור אמת אחד לשדות, כדי ששני המסלולים לא יתפצלו.
function _buildUnifiedSections(r) {
    ...
    return { weights, nutritionDaily, nutritionDetailed, sleepRecovery, workouts, memoryBox };
}

function exportUnifiedData(range) {
    const r = _unifiedRange(range);
    if (r === null) return;
    const { weights, nutritionDaily, nutritionDetailed, sleepRecovery, workouts, memoryBox } = _buildUnifiedSections(r);
    ...
    const payload = {
        ...
        readme: _NUTRI_EXPORT_README.concat(_UNIFIED_README_EXTRA),
...
// ואסור להסתיר אותו.
// wristTempDev מחזיק כעת את הטמפ' ה**מוחלטת** מ-Apple (°C, ~30–35), לא סטייה — הסטייה
// מחושבת באפליקציה מול baseline אישי. הטווח בצלזיוס: אם הקיצור ישלח בפרנהייט (~91) הערך
// יסונן בבטחה (טמפ' פשוט לא תשוקלל) במקום להרעיל — כשל בטוח.
const _VITAL_RANGE = {
    hrv:          [5, 260],    // ms
    rhr:          [25, 130],   // bpm
...
function computeReadiness(nights, idx) {
    ...
    // fade-in: לא נכנסת לציון עד שיש ≥14 לילות טמפ' תקינים ב-baseline.
    if (_validVital('wristTempDev', n.wristTempDev)) {
        const bTemp = _recoveryBaseline(nights, idx, 'wristTempDev');
        if (bTemp.med != null && bTemp.n >= TEMP_MIN_NIGHTS) {
            const tSpread = Math.max(bTemp.spread || 0, 0.1);   // רצפת spread ~0.1° — טמפ' יציבה לא תפוצץ z
            const z = _clampZ((n.wristTempDev - bTemp.med) / tSpread);
            const dev = Math.round((n.wristTempDev - bTemp.med) * 10) / 10;
            parts.push({ w: 0.07, contrib: -Math.max(z, 0), label: 'טמפ׳', delta: (dev > 0 ? '+' : '') + dev + '°', dir: z <= 0.5 ? 'up' : 'down', z });
        }
    }
