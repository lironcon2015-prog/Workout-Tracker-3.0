...
// _cleanNutriOutliers — ימים שנמוכים ביותר מ-30% מהממוצע (כנראה הזנה חסרה) מוחלפים
// בממוצע (של שאר הימים), כדי שלא ייצרו ספייקים בגרף. משפיע על הגרף בלבד.
function _cleanNutriOutliers(points) {
    if (points.length < 4) return points;
    const vals = points.map(p => p.val);
    const mean0 = vals.reduce((a, b) => a + b, 0) / vals.length;
    const floor = mean0 * 0.7;                         // 30% מתחת לממוצע
