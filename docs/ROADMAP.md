...
| **מנוע TDEE / מאזן אנרגיה** | חישוב רב-שיטתי (מדידה + Katch/Cunningham/Mifflin), פילוח לשלב, טווח ביטחון, פרופיל גוף, עוגן ל-AI | v15.77–15.79 |
| **Audit באגים עמוק** | 12 תיקונים: quota reporting, אישור שחזור ענן, clamp ל-setIdx, escaping גלובלי, ולידציית תאריכים, SW offline fallback | v15.97 |
| **UI/UX מול מתחרים — Wave 1** | קונטרסט AA, יעדי מגע ≥44px, 100dvh, empty states מעוצבים, חגיגת PR בזמן אמת (e1RM) | v15.98 |
| **UI/UX מול מתחרים — Wave 2** | טבלת סטים חיה + ghost values (Hevy-style), מחשבון פלטות, פירמידת חימום, toggle "מעבר ישיר לתרגיל" | v15.99 |

פירוט טכני מלא של תתי-המערכות האלה: ראה `PROJECT_KNOWLEDGE.md`.

### כיוונים פתוחים שהוצעו (טרם בוצעו)
- דשבורד Cut/Surplus (טיים-ליין משקל×קלוריות×נפח), דוח שבועי הוליסטי מה-AI, יעדי מאקרו עם טבעות אדהרנס.
- **Wave 3 — מוטיבציה (מניתוח ה-UI/UX, v15.99):** כרטיס Streak בבית, דוח שבועי ויזואלי ("השבוע שלי": נפח/PRs/אדהרנס תזונה + נרטיב coach), אבני דרך והישגים.
- **Wave 4 — תוכן ועומק:** תמונת תרגיל במסך האימון, ספריית templates, סוגי סט (drop-set/rest-pause), היקפי גוף.
