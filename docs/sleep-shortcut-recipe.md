...
4. **Find Health Samples** → Type: **Sleep** (Asleep) · Start Date is **today** →
   **Calculate Statistics** → Sum (בדקות) → משתנה `ASLEEP`.
5. **Find Health Samples** → Type: **Resting Heart Rate** · today → Statistics → Average → `RHR`.
6. **Find Health Samples** → Type: **Heart Rate Variability** · today → Average → `HRV`.
7. **Find Health Samples** → Type: **Respiratory Rate** · today → Average → `RESP`.
