...
const StorageManager = {
    ...
    mergeSleepDays(nights) {
        ...
        (nights || []).forEach(d => {
            ...
            // HRV/נשימה הם דגימות ליליות. סנכרון במהלך היום מזהם אותם בערכי-ערוּת
            // (HRV נמוך, נשימה גבוהה) ומוריד את הציון שלא בצדק. לכן: **ערך תקין קיים גובר**
            // — סנכרון מאוחר רק **ממלא** מדד שהיה חסר בבוקר, ואינו דורס ערכי-שינה תקינים.
            //
            // ⚠️ RHR הוצא מהכלל הזה ב-v19.7.5, ובכוונה. הוא אינו דגימה גולמית אלא **מדד
            // מחושב ומסונן של אפל** ("דופק במנוחה"), שאפל מעדכנת לאורך היום ככל שמצטברים
            // נתוני מנוחה — ואין בו זיהום-ערוּת לחשוש ממנו. fill הקפיא את הקריאה המוקדמת
            // של הבוקר, ולכן המספר בגימפרו נשאר חלוק על Health לצמיתות. עכשיו: ערך נכנס
            // תקין גובר (כמו טמפ'), וערך חסר אינו מוחק ערך שנשמר. מזה גם נגזרת היכולת
            // לתקן היסטוריה — Manual Export מ-HAE על טווח ימים ידרוס ערכים שקפאו, וה-baseline
            // (חציון 28 לילות, מחושב בכל רינדור מ-SLEEP_DAILY) מתקן את עצמו מיד אחריו.
            if (existing) {
                ['hrv', 'respRate'].forEach(k => { if (_vitalOk(k, existing[k])) merged[k] = existing[k]; });
                if (!_vitalOk('rhr', d.rhr) && _vitalOk('rhr', existing.rhr)) merged.rhr = existing.rhr;
...
- אל תשווה ביצועים בין ציוד שונה: מוט ≠ משקולות יד ≠ מכונה ≠ כבל, וציוד מזדמן (מלון/נסיעה) ≠ הציוד הקבוע. אם שני סשנים נבדלים בציוד — אל תסיק מהם מגמת כוח או רגרסיה; ציין שההשוואה אינה בת-תוקף. אך אל תעצור שם: השווה כל תרגיל לפעם הקודמת שהוא עצמו בוצע באותו ציוד. לתרגילים תחליפיים (Cable Row / Machine Row, Single Leg Curl / Seated Leg Curl) יש בייסליין נפרד לכל אחד — אל תשווה ביניהם, אך כן השווה כל אחד לבייסליין שלו. "אין השוואה תקפה" אינה תשובה מספקת כשקיים בייסליין לתרגיל.
...
# מדד ההתאוששות — כללי פרשנות
- הציון מלווה תמיד ברצועה מפורשת (נמוך / בינוני / מוכן). השתמש אך ורק בתווית הרצועה שסופקה. אל תתאר ציון בינוני כ"נמוך" או "נמוך במיוחד", ואל תקרא את הציון כאחוז. ציון 36 ברצועה "בינוני" הוא בינוני.
- הכלל המנחה: **אין להחליף מדידה טובה באומדן גרוע.** ה-RIR בסט בפועל הוא מדידה ישירה של מצב המתאמן באותו רגע; ציון ההתאוששות הוא אומדן גס שחושב בבוקר. לכן:
  * **אסור** לגזור מהציון הנחיות לאימון: אל תמליץ להימנע מסט AMRAP, להעלות RIR, להוריד סטים, להפחית נפח או לדלג על אימון בגלל ציון ההתאוששות. ויסות המאמץ נעשה ע"י RIR בזמן אמת.
  * **מותר** להשתמש בציון כהסבר בדיעבד לביצוע חלש מהצפוי (readiness נמוך = אנומליה מוסברת, לא רגרסיה — כמו הערת "ספסל גבוה" או "מחלה"). זו אינה עילה לשינוי TM או תוכנית.
  * **מותר** לציין מגמה: ירידה מתמשכת של 7+ ימים מתחת ל-baseline היא קלט לגיטימי לשקילת דילואוד ברמת השבוע/בלוק — בניגוד לציון יומי בודד.
- חוב השינה המצטבר הוא נתון תצוגה עצמאי ואינו רכיב בציון ההתאוששות. אל תציג אותו כמניע של הציון ואל תגזור ממנו הנחיה. לניתוח אימון — משך השינה בלילה שלפני האימון הוא הרלוונטי, לא הממוצע השבועי.
- אם הציון מבוסס על חלק מהמדדים בלבד (מצוין במפורש) — ציין זאת בקצרה ואל תבנה עליו מסקנה משמעותית.`,
...
התייחס ל: ביצוע מול היעד, חזרות ועומס מול הפעם הקודמת שכל תרגיל **עצמו** בוצע (רק כאשר הציוד זהה — אחרת ציין שאין השוואה תקפה, אך אם קיים בייסליין לאותו תרגיל עצמו, השווה אליו), איכות הסטים (RIR), נקודות חוזק, ונקודה אחת לשיפור בפעם הבאה. **אם אין ממצא ברמת מובהקות בינונית ומעלה — כתוב "אין נקודה לשיפור" ואל תייצר אחת.**
אם מדדי ההתאוששות בבוקר האימון היו נמוכים — שקלל זאת: אל תפרש ביצוע מופחת כירידת כוח אמיתית, וציין שההתאוששות היא ההסבר הסביר.

# כללי ניתוח נוספים
- **סשן המסומן Deload / "מופחת" / נפח מופחת אינו בר-השוואה לסשן רגיל כמגמת כוח.** ציין זאת במפורש, אל תסיק ממנו רגרסיה, ואל תמליץ על שינוי בעקבותיו. תגית השבוע מופיעה בשורת הכותרת של האימון (למשל "Deload"), והערת אימון שמצהירה על הפחתה מכוונת גוברת על כל חישוב נפח.
- **טונאז' הוא מדד עומס מצטבר, לא מדד כוח.** אל תסיק ממנו התקדמות או נסיגה כשהעומס, מספר החזרות או מספר הסטים/סבבים שונים. השוואת נפח תקפה רק במספר סטים או סבבים זהה.
...
"## השוואה לבלוק הקודם" — השווה לשבוע/אימון המקביל בבלוק הקודם והדגש שיפור או נסיגה במספרים (רק בהשוואות בנות-תוקף מבחינת ציוד). **אם אין ממצא ברמת מובהקות בינונית ומעלה — כתוב "אין נקודה לשיפור" ואל תייצר אחת.**
...
"## המלצות לבלוק הבא" — 2-3 המלצות קונקרטיות לתכנון הבלוק הבא (ללא המלצות Training Max). **אם אין ממצא ברמת מובהקות בינונית ומעלה — כתוב "אין נקודה לשיפור" ואל תייצר אחת.**
